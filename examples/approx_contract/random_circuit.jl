...
# function line_partition_gates(N, gates; order="row")
#   @assert order in ["row", "column"]
#   nrow, ncol = N
#   if order == "row"
#     partition_qubits = [[(c - 1) * nrow + r for r in 1:nrow] for c in 1:ncol]
#   else
#     partition_qubits = [[(c - 1) * nrow + r for c in 1:ncol] for r in 1:nrow]
#   end
...
function random_circuit_tn_line_partition_tree(
  ...
  for (i, l) in enumerate(layers)
    ...
    # if i % 4 in [0, 1]
    #   push!(partition_gates, line_partition_gates(N, l; order="row")...)
    # else
    #   push!(partition_gates, line_partition_gates(N, l; order="column")...)
    # end
  end
  hilbert = qubits(prod(N))
  tensor_partitions = [productstate(hilbert)[:]]#[Vector{ITensor}([ITensor([1.0, 0.0], i) for i in hilbert])]
  for gates in partition_gates
    tensors, hilbert = gate_tensors(hilbert, gates)
    push!(tensor_partitions, tensors)
  end
  # tensor_partitions = [vectorize(tensor_partitions)]
  for gates in reverse(partition_gates)
    tensors, hilbert = gate_tensors(hilbert, reverse(gates); applydag=true)
    push!(tensor_partitions, tensors)
  end
  push!(tensor_partitions, productstate(hilbert)[:])#Vector{ITensor}([ITensor([1.0, 0.0], i) for i in hilbert]))
