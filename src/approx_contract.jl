...
function get_tn_cache_sub_info(
  tn_tree::Dict{Vector,ITensor}, cache_binary_trees::Vector
)
  cached_tn = []
...
function orthogonalize!(ctree_to_tn_tree::Dict, environments::Vector, c::Vector)
  @timeit_debug ITensors.timer "orthogonalize" begin
    ...
    for env in environments
      ortho_tensors = Vector{ITensor}([tensor_to_ortho_tensor[t] for
        t in get_child_tn(ctree_to_tn_tree, env)
      ])
      ctree_to_tn_tree[env] = ortho_tensors
    end
    push!(
      ctree_to_tn_tree[c[index[1]]], tensor_to_ortho_tensor[source_tensor],
    )
  end
end
...
function approximate_contract(
  ...
  @timeit_debug ITensors.timer "approximate_contract" begin
    ...
    ctree_to_tn_tree = Dict{
      Vector,Union{Dict{Vector,ITensor},Vector{ITensor}}
    }()
    # accumulate norm
