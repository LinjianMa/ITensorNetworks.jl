...
function _contract_deltas(partition::DataGraph; root=1)
  ...
  rootinds = _get_out_inds(subgraph(partition, nonleaves))
  # check rootinds are not noncommoninds of the partition
  @assert intersect(rootinds, _get_out_inds(partition)) == []
  nonleaves_tn = reduce(union, [partition[v] for v in nonleaves])
  nonleaves_tn = _contract_deltas(nonleaves_tn)
  ...
  deltas = Vector{ITensor}([nonleaves_tn[v] for v in delta_vs])
  ds = _delta_inds_disjointsets(deltas, Vector{Index}())
  deltainds = [ds...]
