...
function _constrained_minswap_inds_ordering(
  constraint_tree::DataGraph, ref_ordering::Vector, tn::ITensorNetwork
)
  @timeit_debug ITensors.timer "_constrained_minswap_inds_ordering" begin
    leaves = leaf_vertices(constraint_tree)
    root = _root(constraint_tree)
    v_to_order = Dict{Integer,Vector{Set}}()
    for v in post_order_dfs_vertices(constraint_tree, root)
      if v in leaves
        v_to_order[v] = [constraint_tree[v][1]...]
        continue
      end
      child_orders = Vector{Vector{Set}}()
      children = child_vertices(constraint_tree, v)
      for inds_tuple in constraint_tree[v][1]
        cs = filter(c -> constraint_tree[c][1] == inds_tuple, children)
        @assert length(cs) == 1
      ...
      if constraint_tree[v][2] == "ordered"
