...
function _binary_partition_with_truncate!(
  ...
  tree = dfs_tree(partition, root)
  @assert length(child_vertices(tree, root)) == 1
  for v in post_order_dfs_vertices(tree, root)
    children = sort(child_vertices(tree, v))
...
function approx_itensornetwork!(
  ...
  @assert length(vertices(binary_tree_partition)) == 1
  add_vertex!(output_tn, root)
  root_tensor = _optcontract(Vector{ITensor}(binary_tree_partition[root]))
